...
1. Расширить покрытие API:
   добавить tools `get_api_summary`, `get_service_status`, `get_ticket_url`, `get_bus_station_calendar`.

2. Нормализовать ответы:
   добавить компактные структуры для AI-клиентов, чтобы они могли отвечать человеку без разбора сырых JSON-полей.

3. Улучшить поиск:
   добавить `search_everything` по остановкам, маршрутам и направлениям автовокзала одним вызовом.

4. Поддержать сложные маршруты:
   после реализации в основном `transport12` добавить tool для построения маршрута с пересадками.

5. Добавить проверки:
   smoke-тесты tools против тестового `TRANSPORT12_API_BASE_URL` и CI-сборку.
